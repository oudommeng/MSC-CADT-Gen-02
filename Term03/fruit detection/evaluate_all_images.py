...
# Define architecture classes exactly as in notebook
class Anchors:
...
# Let's search over the first 50 images to see if there are high confidence detections
img_paths = sorted(glob.glob("images/*.png"))[:50]
best_images = []
...
for idx, ip in enumerate(img_paths):
    ...
    class_scores = predictions[0, :, 5:]
    probs = class_scores * obj_scores.unsqueeze(-1)
    max_probs, class_idxs = probs.max(dim=-1)
    
    max_prob = max_probs.max().item()
    best_idx = max_probs.argmax().item()
    best_box = predictions[0, best_idx, 1:5]
    best_class = class_idxs[best_idx].item()
    
    print(f"Image {os.path.basename(ip)}: Max Prob = {max_prob:.4f} | Class = {best_class} | Box = {best_box.tolist()}")
    if max_prob > 0.4:
        best_images.append((ip, max_prob))

print("\nImages with confidence > 0.4:", len(best_images))
for ip, prob in best_images:
    print(f"  {ip}: {prob:.4f}")
